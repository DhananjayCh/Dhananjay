...
function oprnPopupByClick(tab_Clicked, dpData, dpData1) {
    ...
    if (popupClick.length > 0) {
        ...
        inilizeRichTxetEditorInput("summernote");
        bindFancyUpload();
        inilizeColorPicker();
        inilizeFileUploader();
        iniliazeSelect2();
...
function bindEdit(data, bindFor) {
    ...
    } else if (bindFor == "Article_Setting") {
        if (data.Show_Multiple) {
            $('#Show_multiple_Data').addClass('on')
        }
    } else if (bindFor == "Notice_Setting") {
        if (data.Show_Multiple) {
            $('#Show_multiple_Data').addClass('on')
        }
...
function triggerNotify(message,position,type) {
    notif({
        type: type,
...
function triggerNotifyConfirm(message, functionName) {
...
function inilizeRichTxetEditorInput(Id) {
    $('#'+Id).summernote({
